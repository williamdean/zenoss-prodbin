...
class ZopeRequestLogger(object):
    ...
    def __init__(self, filename = DEFAULT_LOG_FILE):
        ...
        self.zope_logging_active = os.path.isfile(ZopeRequestLogger.CONFIG_FILE)
        self._redis_last_connection_attemp = time.time()
        if self.zope_logging_active:
    ...
    def _load_data_to_log(self, request):
        ...
        fingerprint = [ data.get(field, '') for field in ZopeRequestLogger.FINGERPRINT_FIELDS ]
        fingerprint = '{0}:{1}'.format(ZopeRequestLogger.REDIS_KEY_PATTERN, ZopeRequestLogger.SEPARATOR.join(fingerprint))
    ...
    def log_request(self, request, finished=False):
        ''' '''
        if not self.zope_logging_active:
            return
        ...
        if self._redis_client:
            try:  # Tests the redis connection
                ...
                ZopeRequestLogger.LOG.info("Connection to redis lost")
                self._redis_client = None
            else:
